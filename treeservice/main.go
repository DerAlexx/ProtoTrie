...
/*
Token is a token like a API-key.
*/
type Token string
...
/*
RootNodes is a Map of rootnodes containing all roots --> Sorted by Key (Trie ID)
and Value (Root Node --> actor.Actor).
*/
...
func (*ServerRemoteActor) Receive(context actor.Context) {
	...
	case *tree.WantBasicNodeActorsMessage:
		...
		propright := actor.PropsFromProducer(func() actor.Actor {
			return tree.CreateBasicNode(size)
		})
...
/*
NewServerRemoteActor will create and return a ServerRemoteActor.
*/
...
/*
AddNewTrie will add a rootNode into the map and return the ID and the token of the new Trie.
*/
...
/*
main starts an actorsystem that can be reached remotely.
*/
func main() {
	...
	PrintLog("The Server will listen on Port 8091")
